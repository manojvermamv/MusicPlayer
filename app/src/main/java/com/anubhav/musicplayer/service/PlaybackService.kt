...
class PlaybackService : MediaSessionService(), MediaSession.Callback {
    ...
    companion object {
        const val SEEK_FORWARD = 5000
        const val SEEK_BACKWARD = 5000
    ...
    override fun onCustomCommand(
        ...
        if (customCommand.customAction == NotificationPlayerCustomCommandButton.REWIND.customAction) {
            if (currentSongPosition - SEEK_BACKWARD >= 0) {
                session.player.seekTo(currentSongPosition - SEEK_BACKWARD)
            } else {
                session.player.seekTo(0)
            }
        }
        if (customCommand.customAction == NotificationPlayerCustomCommandButton.FORWARD.customAction) {
            if (currentSongPosition + SEEK_FORWARD <= session.player.duration) {
                session.player.seekTo(currentSongPosition + SEEK_FORWARD)
            } else {
